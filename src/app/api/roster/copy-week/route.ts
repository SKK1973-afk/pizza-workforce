import { NextResponse } from 'next/server';
import { createServiceClient } from '@/lib/supabase/server';
import { getApiUser } from '@/lib/api-auth';
import { canEditRoster } from '@/lib/permissions';
import { addDays, format, parseISO } from 'date-fns';

export async function POST(request: Request) {
  const auth = await getApiUser();
  if (auth instanceof NextResponse) return auth;
  const { user } = auth;

  const { store_id, week_start_date } = await request.json();
  if (!store_id || !week_start_date) {
    return NextResponse.json({ error: 'store_id and week_start_date required' }, { status: 400 });
  }
  if (!canEditRoster(user, store_id)) {
    return NextResponse.json({ error: 'Forbidden' }, { status: 403 });
  }

  const supabase = await createServiceClient();
  const prevWeek = format(addDays(parseISO(week_start_date), -7), 'yyyy-MM-dd');

  const { data: prevShifts, error: fetchError } = await supabase
    .from('shifts')
    .select('*')
    .eq('store_id', store_id)
    .eq('week_start_date', prevWeek);

  if (fetchError) return NextResponse.json({ error: fetchError.message }, { status: 500 });
  if (!prevShifts?.length) {
    return NextResponse.json({ error: 'No shifts found for previous week' }, { status: 404 });
  }

  const newShifts = prevShifts.map((s) => ({
    store_id: s.store_id,
    user_id: s.user_id,
    shift_date: format(addDays(parseISO(s.shift_date), 7), 'yyyy-MM-dd'),
    scheduled_start: s.scheduled_start,
    scheduled_end: s.scheduled_end,
    shift_type: s.shift_type,
    week_start_date,
    notes: s.notes,
    created_by: user.id,
    published: false,
  }));

  const { data, error } = await supabase.from('shifts').insert(newShifts).select('id');
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });

  return NextResponse.json({ copied: data?.length ?? 0 });
}
